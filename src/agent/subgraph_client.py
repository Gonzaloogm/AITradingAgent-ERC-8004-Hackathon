...
class SubgraphClient:
  ...
  def __init__(
      ...
      if api_key and api_key in self.subgraph_url:
          ...
          print(f"  Subgraph client initialized WITHOUT API key - queries will fail")
  ...
  async def get_agent_by_owner(self, owner_address: str) -> Optional[Dict[str, Any]]:
      ...
      try:
          ...
          if "401" in error_msg or "Unauthorized" in error_msg:
              print(f"  Subgraph auth failed - check SUBGRAPH_API_KEY")
          else:
          ...
          if "api-key" in error_msg.lower() or "api_key" in error_msg.lower():
              print(f"  Subgraph requires API key - set SUBGRAPH_API_KEY env var")
          else:
