...
import { Wallet, ArrowUpCircle, Activity, Info } from 'lucide-react';
...
export default function LiquidityPage() {
  ...
  const handleConnect = async () => {
    try {
      ...
      setConnected(true);
      toast('Authorization linked', 'success');
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      toast('Ledger settlement committed', 'success');
  ...
  return (
    <div className="space-y-6 animate-fadein max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[380px]">
        ...
        {/* INBOUND PATHWAY */}
        <div className="dashboard-card p-6 flex flex-col justify-between border-teal-500/10 active:border-teal-500/20">
          ...
          <div className="flex gap-2 mt-4">
             <button onClick={() => { navigator.clipboard.writeText(wallet?.address); toast('Path copied', 'info'); }} className="flex-1 bg-white/5 hover:bg-white/10 text-slate-400 py-3 rounded text-[10px] font-bold uppercase transition-all tracking-widest">Copy ID</button>
      ...
      {connected && (
        <div className="dashboard-card p-10 flex flex-col lg:flex-row items-center justify-between gap-10 animate-fadein border-[#00BFA5]/10">
          ...
          <div className="flex items-center gap-8 relative">
             ...
             <div className="flex flex-col items-end">
                ...
                <input
                   ...
                   className="bg-black/30 border border-white/10 rounded px-4 py-2 text-sm text-[#00BFA5] w-28 text-right font-mono focus:outline-none focus:border-[#00BFA5]/20"
                />
             </div>
             <button
